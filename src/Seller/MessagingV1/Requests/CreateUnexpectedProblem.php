...
use SellingPartnerApi\Seller\MessagingV1\Responses\CreateUnexpectedProblemResponse;

/**
 * createUnexpectedProblem
 */
class CreateUnexpectedProblem extends Request implements HasBody
{
    use HasJsonBody;
    ...
    public function createDtoFromResponse(Response $response): CreateUnexpectedProblemResponse
    {
        $status = $response->status();
        $responseCls = match ($status) {
            201, 400, 403, 404, 413, 415, 429, 500, 503 => CreateUnexpectedProblemResponse::class,
