...
use SellingPartnerApi\Seller\FBAInboundV20240320\Responses\GenerateTransportationOptionsResponse;

/**
 * generateTransportationOptions
 */
class GenerateTransportationOptions extends Request implements HasBody
{
    use HasJsonBody;
    ...
    public function createDtoFromResponse(Response $response): GenerateTransportationOptionsResponse|ErrorList
    {
        $status = $response->status();
        $responseCls = match ($status) {
            202 => GenerateTransportationOptionsResponse::class,
    ...
    {
        return $this->generateTransportationOptionsRequest->toArray();
    }
}
