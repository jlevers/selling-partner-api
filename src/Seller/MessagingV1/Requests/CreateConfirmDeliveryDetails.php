...
use SellingPartnerApi\Seller\MessagingV1\Responses\CreateConfirmDeliveryDetailsResponse;

/**
 * createConfirmDeliveryDetails
 */
class CreateConfirmDeliveryDetails extends Request implements HasBody
{
    use HasJsonBody;
    ...
    public function createDtoFromResponse(Response $response): CreateConfirmDeliveryDetailsResponse
    {
        $status = $response->status();
        $responseCls = match ($status) {
            201, 400, 403, 404, 413, 415, 429, 500, 503 => CreateConfirmDeliveryDetailsResponse::class,
