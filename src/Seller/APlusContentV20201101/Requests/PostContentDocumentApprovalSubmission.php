...
class PostContentDocumentApprovalSubmission extends Request implements HasBody
{
    ...
    public function createDtoFromResponse(Response $response): PostContentDocumentApprovalSubmissionResponse|ErrorList
    {
        $status = $response->status();
        $responseCls = match ($status) {
            200 => PostContentDocumentApprovalSubmissionResponse::class,
